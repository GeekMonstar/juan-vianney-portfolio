import { icons, competences, projectsData, technos, T_skill } from "./data.js";
...
function start() {
  ...
  setTimeout(() => {
    for (let i = 0; i < competences.length; i++) {
      ...
      progressValues[i].setAttribute(
        "style",
        `width:${competences[i].level}%;height:100%`
      );
...
function showIcon(type: string, name: T_skill) {
  if (type === "skills") {
